# This file set up the logic of the design from WorkToRest.py, we will start timers and save data to file "data.oil"
...
class window(QtWidgets.QDialog):
    ...
    def run_watch_twt(self): # setting up the timer
        self.ui.savedata.setDisabled(False)
        self.mscounter_twt = self.mscounter_twt + 1  # making counter for our timer
        self.stopwatch_twt()

    def stopwatch_twt(self): # calculation difference between counter and now time
        self.text_twt = str(datetime.timedelta(milliseconds=self.mscounter_twt))[:-7] # main calculations. You be able to edit format of time by changing
    ...
    def startTwt(self): # launch main process for TWT timer
    ...
    def stopTwt(self): # stop the timer not reload it
        self.timer_twt.stop()
    ...
    def run_watch_trt(self): # setting up the timer
        self.ui.savedata.setDisabled(False)
        self.mscounter_trt = self.mscounter_trt + 1
        self.stopwatch_trt()

    def stopwatch_trt(self): # calculation difference between counter and now time
    ...
    def startTrt(self): # launch main process for TRT timer
    ...
    def stopTrt(self): # stop the timer not reload it
        self.timer_trt.stop()
        self.ui.stop_trt.setDisabled(True)

    def savedata(self): # saving data and check status of file
        if self.text_trt == '' and self.text_twt == '': # if user don't launch both - timer's program won't record data
            return
        # stop all timers
        self.timer_twt.stop()
        self.timer_trt.stop()
...
# note about "\n": we need to create "\n\n" (further Enter) for indicating the new block of data.
    ...
    def show_plots(self): # making and showing plots
        from plots import main_graph # import another file with code
